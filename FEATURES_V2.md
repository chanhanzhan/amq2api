# AMQ2API v2.0 - 完整功能文档

## 新功能概览

AMQ2API v2.0 引入了以下重大新功能：

### 1. 账号池管理 (Account Pool Management)
- 支持管理多个 Amazon Q 账号
- 自动负载均衡和轮询
- 实时健康检查
- 使用统计和监控

### 2. API 密钥认证 (API Key Authentication)
- 安全的 API 密钥管理
- 兼容 Claude 协议（支持 Authorization 和 x-api-key 头）
- 细粒度的访问控制
- 速率限制（每分钟/每天）

### 3. OpenAI API 格式支持 (OpenAI API Compatibility)
- 完整支持 OpenAI chat completion 格式
- 自动格式转换
- 模型名称映射
- 流式响应支持

### 4. Web 管理界面 (Web Admin Interface)
- 现代化的管理面板
- 账号管理
- API 密钥管理
- 实时统计和监控

---

## 快速开始

### 安装依赖

```bash
pip install -r requirements.txt
```

### 初始化数据库

数据库会在首次启动时自动创建。默认位置：`data/amq2api.db`

### 启动服务

```bash
# 使用新的应用文件
python app_new.py

# 或使用 uvicorn
uvicorn app_new:app --host 0.0.0.0 --port 8080
```

### 访问管理界面

打开浏览器访问：`http://localhost:8080/admin/dashboard`

**首次访问：**
系统会提示您输入管理员 API 密钥。

**默认管理员密钥：**
```
amq-admin-default-key-change-immediately
```

⚠️ **重要：** 登录后请立即创建新的管理员密钥并删除默认密钥！

---

## 管理员认证

### API 密钥认证

v2.0 使用统一的 API 密钥系统进行认证，不再使用单独的管理员账号。

**API 密钥类型：**

1. **普通 API 密钥** (`is_admin=false`)
   - 可以访问 `/v1/messages` 和 `/v1/chat/completions`
   - 不能访问管理面板

2. **管理员 API 密钥** (`is_admin=true`)
   - 可以访问所有 API 端点
   - 可以访问管理面板
   - 可以管理账号和 API 密钥

### 默认管理员密钥

系统首次启动时会自动创建默认管理员 API 密钥：
```
amq-admin-default-key-change-immediately
```

### 创建新的管理员密钥

登录管理面板后：

1. 点击"API密钥"标签
2. 点击"+ 创建密钥"
3. 填写密钥信息
4. **勾选"管理员权限"** 复选框
5. 点击创建
6. 立即保存新密钥（只显示一次）
7. 删除默认管理员密钥

或使用 API：

```bash
curl -X POST http://localhost:8080/admin/api-keys \
  -H "Authorization: Bearer amq-admin-default-key-change-immediately" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "我的管理员密钥",
    "description": "个人使用",
    "is_admin": true,
    "requests_per_minute": 100,
    "requests_per_day": 10000
  }'
```

### Web 界面存储

- API 密钥存储在浏览器 localStorage 中
- 每次请求自动添加 Authorization 头
- 点击"清除密钥"按钮可以更换密钥

### 安全特性

- ✅ 统一的 API 密钥认证系统
- ✅ 密钥格式：`amq-` + 48字节随机字符串
- ✅ 细粒度的权限控制（is_admin 标志）
- ✅ 速率限制（每分钟/每天）
- ✅ 密钥过期支持
- ✅ 无状态认证，无需session管理

---

## 账号池管理

### 添加账号

通过 Web 界面或 API 添加账号：

**通过 Web 界面：**
1. 访问管理面板
2. 点击"账号管理"
3. 点击"+ 添加账号"
4. 填写账号信息

**通过 API：**

```bash
curl -X POST http://localhost:8080/admin/accounts \
  -H "Content-Type: application/json" \
  -d '{
    "name": "账号1",
    "refresh_token": "your_refresh_token",
    "client_id": "your_client_id",
    "client_secret": "your_client_secret",
    "profile_arn": null,
    "requests_per_minute": 10,
    "notes": "测试账号"
  }'
```

### 账号轮询机制

- 使用 Round-Robin 算法
- 优先选择 RPM 最低的账号
- 自动跳过不健康的账号
- 支持每账号 RPM 限制

### 健康检查

系统会自动跟踪账号健康状态：
- 成功的请求标记账号为健康
- 失败的请求标记账号为不健康
- 不健康的账号会被自动跳过

---

## API 密钥管理

### 创建 API 密钥

**通过 Web 界面：**
1. 访问管理面板
2. 点击"API密钥"
3. 点击"+ 创建密钥"
4. 填写密钥信息
5. **重要：** 创建后立即保存密钥，密钥只显示一次！

**通过 API：**

```bash
curl -X POST http://localhost:8080/admin/api-keys \
  -H "Content-Type: application/json" \
  -d '{
    "name": "测试密钥",
    "description": "用于测试",
    "requests_per_minute": 60,
    "requests_per_day": 10000,
    "expires_days": null
  }'
```

响应示例：
```json
{
  "id": 1,
  "key": "amq-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
  "name": "测试密钥",
  "is_active": true,
  ...
}
```

### 使用 API 密钥

支持两种方式传递 API 密钥：

**方式 1: Authorization Header（推荐）**

```bash
curl -X POST http://localhost:8080/v1/messages \
  -H "Authorization: Bearer amq-your-api-key-here" \
  -H "Content-Type: application/json" \
  -d '{...}'
```

**方式 2: x-api-key Header**

```bash
curl -X POST http://localhost:8080/v1/messages \
  -H "x-api-key: amq-your-api-key-here" \
  -H "Content-Type: application/json" \
  -d '{...}'
```

### 速率限制

每个 API 密钥支持：
- 每分钟请求数限制（RPM）
- 每天请求数限制（RPD）
- 自动重置计数器

---

## OpenAI API 支持

### 端点

```
POST /v1/chat/completions
```

### 请求格式

```bash
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "Authorization: Bearer amq-your-api-key" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-4",
    "messages": [
      {
        "role": "system",
        "content": "You are a helpful assistant"
      },
      {
        "role": "user",
        "content": "Hello!"
      }
    ],
    "temperature": 0.7,
    "max_tokens": 1000,
    "stream": true
  }'
```

### 模型映射

| OpenAI 模型 | Claude 模型 |
|-------------|-------------|
| gpt-4 | claude-sonnet-4 |
| gpt-4-turbo | claude-sonnet-4.5 |
| gpt-4o | claude-sonnet-4.5 |
| gpt-4o-mini | claude-sonnet-4 |
| gpt-3.5-turbo | claude-sonnet-4 |

### 支持的功能

- ✅ 流式响应
- ✅ 系统消息
- ✅ 多轮对话
- ✅ 工具/函数调用
- ✅ Temperature 控制
- ✅ Max tokens 限制

---

## Claude API 支持

原有的 Claude API 端点仍然完全支持，现在需要 API 密钥认证：

```bash
curl -X POST http://localhost:8080/v1/messages \
  -H "Authorization: Bearer amq-your-api-key" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "claude-sonnet-4.5",
    "messages": [
      {
        "role": "user",
        "content": "Hello!"
      }
    ],
    "max_tokens": 4096,
    "stream": true
  }'
```

---

## 管理 API

### 账号管理

- `GET /admin/accounts` - 列出所有账号
- `POST /admin/accounts` - 创建账号
- `GET /admin/accounts/{id}` - 获取账号详情
- `PUT /admin/accounts/{id}` - 更新账号
- `DELETE /admin/accounts/{id}` - 删除账号

### API 密钥管理

- `GET /admin/api-keys` - 列出所有密钥
- `POST /admin/api-keys` - 创建密钥
- `GET /admin/api-keys/{id}` - 获取密钥详情
- `PUT /admin/api-keys/{id}` - 更新密钥
- `DELETE /admin/api-keys/{id}` - 删除密钥
- `POST /admin/api-keys/{id}/revoke` - 吊销密钥

### 统计信息

- `GET /admin/stats/accounts` - 账号统计
- `GET /admin/stats/api-keys` - API密钥统计

---

## 配置说明

### 环境变量

```bash
# 数据库路径（可选）
DATABASE_PATH=data/amq2api.db

# 服务端口
PORT=8080

# Amazon Q API 端点（可选）
AMAZONQ_API_ENDPOINT=https://q.us-east-1.amazonaws.com/

# Token 端点（可选）
AMAZONQ_TOKEN_ENDPOINT=https://oidc.us-east-1.amazonaws.com/token
```

注意：使用账号池模式时，不再需要全局的 `AMAZONQ_REFRESH_TOKEN` 等环境变量。

### 数据库

- 默认使用 SQLite
- 数据库文件：`data/amq2api.db`
- 自动创建表结构
- 支持热备份

---

## 安全建议

### API 密钥安全

1. **妥善保管密钥**
   - 密钥创建后只显示一次
   - 不要在代码中硬编码密钥
   - 使用环境变量存储密钥

2. **设置合理的速率限制**
   - 根据实际需求设置 RPM/RPD
   - 防止滥用和过度使用

3. **定期轮换密钥**
   - 定期吊销旧密钥
   - 创建新密钥替换

### 账号安全

1. **分散风险**
   - 使用多个账号
   - 设置合理的 RPM 限制

2. **监控健康状态**
   - 定期检查账号健康
   - 及时处理异常账号

3. **备份数据库**
   - 定期备份 `data/amq2api.db`
   - 避免数据丢失

---

## 迁移指南

### 从 v1.0 迁移到 v2.0

1. **安装新依赖**
   ```bash
   pip install -r requirements.txt
   ```

2. **初始化数据库**
   ```bash
   python -c "from app.models.database import init_db; init_db()"
   ```

3. **添加账号**
   - 通过 Web 界面添加原有的 Amazon Q 账号
   - 或使用 API 批量导入

4. **创建 API 密钥**
   - 为现有客户端创建 API 密钥
   - 更新客户端配置以使用新密钥

5. **更新启动脚本**
   ```bash
   # 旧版本
   python main.py
   
   # 新版本
   python app_new.py
   ```

6. **测试功能**
   - 验证账号池工作正常
   - 测试 API 密钥认证
   - 确认 OpenAI API 兼容性

---

## 故障排查

### 问题：无法访问管理界面

**检查：**
- 服务是否正常启动
- 端口是否被占用
- 防火墙设置

### 问题：API 密钥认证失败

**检查：**
- 密钥是否正确
- 密钥是否已激活
- 密钥是否已过期
- 是否超出速率限制

### 问题：没有可用账号

**检查：**
- 是否已添加账号
- 账号是否激活
- 账号是否健康
- 账号是否超出 RPM 限制

### 问题：数据库错误

**解决方案：**
1. 备份现有数据库
2. 删除数据库文件
3. 重新初始化
4. 重新导入数据

---

## 性能优化

### 账号池优化

- 根据实际负载调整账号数量
- 合理设置每账号 RPM 限制
- 定期清理不活跃账号

### 数据库优化

- 定期清理旧的使用日志
- 为常用查询添加索引
- 考虑迁移到 PostgreSQL（大规模部署）

### 缓存优化

- Token 缓存（已实现）
- 响应缓存（可选）
- 会话缓存（可选）

---

## 开发说明

### 项目结构

```
amq2api/
├── app/
│   ├── core/           # 核心功能模块
│   │   ├── account_pool.py
│   │   ├── api_keys.py
│   │   ├── auth_middleware.py
│   │   └── openai_converter.py
│   ├── models/         # 数据模型
│   │   └── database.py
│   ├── api/            # API 端点
│   │   └── admin.py
│   └── web/            # Web 界面
│       ├── static/
│       └── templates/
├── app_new.py          # 新版主应用
├── main.py             # 旧版主应用（保留）
├── config.py
├── auth.py
├── converter.py
├── parser.py
└── ...
```

### 扩展开发

参考各模块的代码和注释，可以轻松扩展：
- 添加新的认证方式
- 支持更多 AI 模型
- 自定义负载均衡策略
- 添加更多统计指标

---

## 更新日志

### v2.0.0 (2024-11)

**新功能：**
- ✅ 账号池管理系统
- ✅ API 密钥认证
- ✅ OpenAI API 兼容性
- ✅ Web 管理界面
- ✅ 使用统计和监控

**改进：**
- 📈 更好的负载均衡
- 🔒 增强的安全性
- 📊 实时监控和统计
- 🎨 现代化的 UI

**向后兼容：**
- ✅ 保留原有 Claude API 支持
- ✅ 保留原有配置方式（可选）

---

## 支持和贡献

- 问题反馈：GitHub Issues
- 功能建议：GitHub Discussions
- 贡献代码：Pull Requests

---

## 许可证

MIT License
